...
'''
@ 此代码，加入“复制”步骤，每次迭代，选取50%的优秀染色体（即适应度最高的染色体） 176、179、192
@ 对已经选择的亲本染色体，再利用轮盘选择-->>交叉-->>变异-->>复制亲本染色体-->>变异+复制-->>子代，即要循环的未经初始化的种群
@ 代码结构更加复杂，但是算法复杂度低，实际的运行效率更高
@ version: 0.2
@ author: jing
@ data: 2018/08/10
'''
...
def selection(population, probabi_value):
    ...
    for i in range(len(probabi_value)):
        ...
        if rand <= probabi_sum[0]:
            result.append(population[0])
        else:
            # result.append([population[i] for i in range(1, len(probabi_sum)) if probabi_sum[i - 1] < rand <= probabi_sum[i]][0])
            for i in range(1, len(probabi_value)):
                if probabi_sum[i - 1] < rand and rand <= probabi_sum[i]:
...
if __name__ == '__main__':
    ...
    while count < 100:
        ...
        probability_value = probability(fitness_list[:int(number / 2)])
        # print("概率：", probability_value)

        select_result = selection(population[:int(number / 2)], probability_value)      # 选择50%适应度较高的种群
        ...
        # 复制
        mutat_result = change_list_three_to_two(population, num = len(array[0]) - 1)[:int(number / 2)] + mutat_result
